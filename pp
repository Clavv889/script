...
local isFetchingItems = false
local originalPlayerPosition = nil
local itemQueue = {}
local stopRequested = false
local isProcessing = false
local RequestStartDraggingItem = nil
local RequestBagDropItem = nil
local RequestBagStoreItem = nil

-- Initialize RemoteEvents for fetch system
spawn(function()
    if ReplicatedStorage:FindFirstChild("RemoteEvents") then
        local RemoteEventsFolder = ReplicatedStorage.RemoteEvents
        RequestStartDraggingItem = RemoteEventsFolder:FindFirstChild("RequestStartDraggingItem")
        RequestBagDropItem = RemoteEventsFolder:FindFirstChild("RequestBagDropItem")  
        RequestBagStoreItem = RemoteEventsFolder:FindFirstChild("RequestBagStoreItem")
    end
end)
...
-- Enhanced function to find only REAL pickupable items
local function isItemPickupable(item)
    ...
    if item:FindFirstChild("Pickupable") or item:FindFirstChild("CanPickup") then
        isPickupable = true
    end
    
    if item:FindFirstChildOfClass("Configuration") then
        isPickupable = true
    end
    
    local scripts = item:GetDescendants()
    for _, obj in pairs(scripts) do
        if obj:IsA("Script") or obj:IsA("LocalScript") then
            if string.find(obj.Name:lower(), "pickup") or string.find(obj.Name:lower(), "drag") then
                isPickupable = true
                break
            end
        end
    ...
    if item:FindFirstChild("ItemType") or item:FindFirstChild("Type") then
        isPickupable = true
    end
    ...
    if item:IsA("Model") then
        ...
        for _, part in pairs(item:GetChildren()) do
            if part:IsA("BasePart") and not part.CanCollide == false and part.Anchored == false then
                hasSolidPart = true
                break
            end
        end
        if hasSolidPart then
            isPickupable = true
        end
    end
...
-- Find Items in Workspace.Items (Enhanced with pickup validation)
local function findAllRealItems(targetItemName)
    local foundItems = {}
    local itemsFolder = Workspace:FindFirstChild("Items")
        ...
        if item.Name == targetItemName then
            ...
            if item:IsA("Model") then
                ...
                elseif item:FindFirstChildOfClass("Part") then
                    position = item:FindFirstChildOfClass("Part").Position
                elseif item:FindFirstChild("Handle") then
                    position = item.Handle.Position
                end
            elseif item:IsA("Part") or item:IsA("BasePart") then
                position = item.Position
            end
            
            if position and isItemPickupable(item) then
...
-- Get the best available bag/sack
local function getPlayerBag()
    local inventory = Player:FindFirstChild("Inventory")
    if not inventory then return nil end
    
    local sackPriority = {"Giant Sack", "Good Sack", "Old Sack"}
    
    for _, sackName in pairs(sackPriority) do
        local sack = inventory:FindFirstChild(sackName)
        if sack then
            return sack
        end
    end
    ...
    for _, item in pairs(inventory:GetChildren()) do
        if string.find(item.Name:lower(), "sack") then
            return item
        end
    end
...
-- Count items in all sacks
local function getItemCountInInventory(itemName)
    local count = 0
    local inventory = Player:FindFirstChild("Inventory")
    if not inventory then return 0 end
    
    for _, sack in pairs(inventory:GetChildren()) do
        if string.find(sack.Name:lower(), "sack") then
...
-- Enhanced Item Pickup with Better Validation
local function attemptItemPickup(itemData)
    local char = Player.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return false end
    ...
    if not isItemPickupable(item) then
        return false, "not_pickupable"
    end
    
    local initialCount = getItemCountInInventory(itemData.name)
    ...
    teleportCharacterTo(itemData.position)
    wait(0.3)
    
    -- Method 1: Try clicking if ClickDetector exists
    ...
    if clickDetector then
        ...
        local newCount = getItemCountInInventory(itemData.name)
        if newCount > initialCount then
            return true
        end
    end
    
    -- Method 2: Use RequestStartDraggingItem
    ...
    -- Method 3: Direct move as fallback
    pcall(function()
        if item.Parent == Workspace.Items then
            item.Parent = bag
            wait(0.3)
        end
    end)
    
    local finalCount = getItemCountInInventory(itemData.name)
...
-- Check if item is in any sack
local function isItemInAnySack(itemName)
    local inventory = Player:FindFirstChild("Inventory")
    if not inventory then return false end
    
    for _, sack in pairs(inventory:GetChildren()) do
        if string.find(sack.Name:lower(), "sack") then
            if sack:FindFirstChild(itemName) then
                return true, sack
            end
        end
    end
...
-- Drop Item at Base
local function dropItemAtBase(itemName)
    local itemFound, containingSack = isItemInAnySack(itemName)
    if not itemFound then return false end
    
    local itemToMove = containingSack:FindFirstChild(itemName)
    if not itemToMove then return false end
    
    pcall(function()
        local dropPosition = originalPlayerPosition + Vector3.new(
            math.random(-3, 3), 2, math.random(-3, 3)
        )
        
        if RequestBagDropItem then
            RequestBagDropItem:FireServer(containingSack, itemToMove, true)
            wait(0.5)
        else
            itemToMove.Parent = Workspace.Items
        end
        
        local itemsFolder = Workspace:FindFirstChild("Items")
        if itemsFolder then
            local droppedItem = itemsFolder:FindFirstChild(itemName)
            if droppedItem then
                if droppedItem:IsA("Model") and droppedItem.PrimaryPart then
                    droppedItem:SetPrimaryPartCFrame(CFrame.new(dropPosition))
                elseif droppedItem:IsA("BasePart") then
                    droppedItem.CFrame = CFrame.new(dropPosition)
                elseif droppedItem:FindFirstChild("Handle") then
                    droppedItem.Handle.CFrame = CFrame.new(dropPosition)
                end
            end
        end
    end)
...
-- Main Fetch Function (replaces bringAllItems)
function fetchAllItems(itemName)
    if isProcessing then
        showNotification("Process already running!", 3)
        return false
    end
    ...
    local bag = getPlayerBag()
    if not bag then
        showNotification("No bag found! You need a sack to store items.", 3)
        return false
    end
    
    originalPlayerPosition = char.HumanoidRootPart.Position
    itemQueue = findAllRealItems(itemName)
    
    if #itemQueue == 0 then
        showNotification("No pickupable " .. itemName .. " found!", 2)
        return false
    end
    ...
    showNotification("Starting fetch: " .. #itemQueue .. " " .. itemName .. "(s) found", 2)
    
    local collectedCount = 0
    local totalItems = #itemQueue
    
    for i, itemData in ipairs(itemQueue) do
        if stopRequested then break end
        
        local success, reason = attemptItemPickup(itemData)
        
        if success then
            showNotification("Returning to base with " .. itemName .. "...", 1)
            teleportCharacterTo(originalPlayerPosition)
            wait(0.3)
            
            if dropItemAtBase(itemName) then
                collectedCount = collectedCount + 1
                showNotification("Dropped " .. itemName .. " at base! (" .. collectedCount .. "/" .. totalItems .. ")", 2)
                wait(0.2)
            end
        end
    ...
    if stopRequested then
        showNotification("Fetch stopped! Collected: " .. collectedCount .. "/" .. totalItems .. " " .. itemName .. "(s)", 3)
    else
        showNotification("Fetch complete! Collected: " .. collectedCount .. "/" .. totalItems .. " " .. itemName .. "(s)", 3)
    end
    
    return collectedCount > 0
end

-- ========== END FETCH SYSTEM FUNCTIONS ==========
...
-- Food Items
local foodItems = {
"Bandage", " Medkit", "Carrot", "Cooked Morsel", "Cooked Steak", "Morsel", "Steak",
"Apple", "Cake", "Ribs", "Chili", "Hearty Stew", "Meat Sandwich"
}
...
showNotification("Fetching " .. itemName .. "...", 2)

spawn(function()
local success = fetchAllItems(itemName)
if success then
...
showNotification(itemName .. " fetch completed!", 2)
else
if _G.selectedItemButton then
...
showNotification("Failed to fetch " .. itemName, 2)
end
...
isFetchingItems = false
stopRequested = true
showNotification("Stopped fetching items", 2)
end)
